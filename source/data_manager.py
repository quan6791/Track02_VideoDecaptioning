...
#v = av.open(path_to_video)
##arr1 = np.asarray((v.demux).decode)
#i = 0
...
def readFrame(path_video):
    video_arr =[]
    v = av.open(path_video)
    for packet in v.demux():
        for frame in packet.decode():
            #if frame.type == 'video':
            img = frame.to_image()  # PIL/Pillow image
...
def readFrameImageIO(path_video):
    video_arr=[]
...
#myclip = VideoFileClip('/home/digit/Desktop/ECCV/Track02_caption/Track2/dataset-sample/train/X/X12.mp4')
        ...
        for i in samples:
            ...
            while len(X) < batchsize:
                ...
                try:
                    ...
                    Xj = list(np.array(readFrame(root_dataset+'/'+part+'/X/X'+str(i+j)+'.mp4'))[idxs])
                    Xj = np.array(Xj, dtype='float32') / 255.
                    #Yj = pims.Video(root_dataset+'/'+part+'/Y/Y'+str(i+j)+'.mp4')[idxs]
                    Yj = list(np.array(readFrame(root_dataset+'/'+part+'/Y/Y'+str(i+j)+'.mp4'))[idxs])
...
def getAllFrames(clipname):
    ...
    try:
        ...
        data = readFrame(root_dataset+'/'+clipname)
 
    except:
       # data = pims.Video(clipname)
       data = readFrame(clipname)
            ...
            while len(X) < different_clips_per_batch:
                ...
                try:
                   # Xj = pims.Video(root_dataset+'/train/X/X'+str(samples[i+j])+'.mp4')[idxs]
                    Xj = list(np.array(readFrameImageIO(root_dataset+'/train/X/X'+str(samples[i+j])+'.mp4'))[idxs])
                    Xj = np.array(Xj, dtype='float32') / 255.
                
                    #Yj = pims.Video(root_dataset+'/train/Y/Y'+str(samples[i+j])+'.mp4')[idxs]
                    Yj = list(np.array(readFrameImageIO(root_dataset+'/train/Y/Y'+str(samples[i+j])+'.mp4'))[idxs])
                    Yj = np.array(Yj, dtype='float32') / 255.
                except:
...
if __name__ == "__main__":
   ...
   # if sys.argv[1] == 'build_and_save_batches': build_and_save_batches(20, 1)
